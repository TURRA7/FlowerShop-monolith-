...
from wtforms import StringField, PasswordField, SubmitField, TextAreaField, FileField, FloatField, SelectField
...
class AddItemForm(FlaskForm):
    name = StringField('Наименование товара', validators=[DataRequired(), Length(min=3, max=15)])
    description = TextAreaField('Описание товара', validators=[DataRequired(), Length(min=3)])
    price = FloatField('Цена', validators=[DataRequired(), NumberRange(min=0)])
    category = SelectField('Категория', choices=[('1', 'Категория 1'), ('2', 'Категория 2'), ('3', 'Категория 3'), ('4', 'Категория 4'), ('5', 'Категория 5'), ('6', 'Категория 6'), ('7', 'Категория 7'), ('8', 'Категория 8')], coerce=int, validators=[DataRequired()])
...
def admin_login():
	...
	if form.validate_on_submit():
		...
		if user and user.password == password:
			...
			flash('Ошибка входа. Проверьте имя пользователя и пароль.', 'error_user')
...
def admin_menu():
	app.logger.info('Входящий запрос: %s %s', request.method, request.path)
	return render_template('admin_menu.html', check_total=Checking.check_admin())
...
def admin_panel():
	...
	if request.method == 'POST':
		if form.validate_on_submit():
			...
			check_name = Checking(name, 'error_panel', 'ОШИБКА: НАЗВАНИЕ ДОЛЖНО ИМЕТЬ ОТ 3-х ДО 15 символов!')
			check_description = Checking(description, 'error_panel', 'ОШИБКА: ОПИСАНИЕ ДОЛЖНО СОСТОЯТЬ МИНИМУМ ИЗ 3-Х СИМВОЛОВ!')
			check_latin = Checking(description, 'error_panel', 'ОШИБКА: ОПИСАНИЕ ДОЛЖНО СОСТОЯТЬ ИЗ РУССКИХ СИМВОЛОВ!')
			check_category = Checking(category, 'error_panel', 'ОШИБКА: НУМЕРАЦИЯ КАТЕГОРИИ ОТ 1 ДО 8!')
			...
			if all(total) and filename != None:
				try:
	...
	return render_template('admin_panel.html', check_total=Checking.check_admin(), form=form)
	...
	if request.method == 'POST':
		if form.validate_on_submit():
			...
			if photo_article:
				...
				photo_article.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
			else:
				filename = None

			check_name_description = Checking(name_article, 'error_article', 'ОШИБКА: НАЗВАНИЕ ДОЛЖНО СОСТОЯТЬ МИНИМУМ ИЗ 3-Х СИМВОЛОВ!')
			check_name_latin = Checking(name_article, 'error_article', 'ОШИБКА: НАЗВАНИЕ ДОЛЖНО СОСТОЯТЬ ИЗ РУССКИХ СИМВОЛОВ!')
			check_text_latin = Checking(text_article, 'error_article', 'ОШИБКА: ОПИСАНИЕ ДОЛЖНО СОСТОЯТЬ ИЗ РУССКИХ СИМВОЛОВ!')
			check_text_description = Checking(text_article, 'error_article', 'ОШИБКА: ОПИСАНИЕ ДОЛЖНО СОСТОЯТЬ МИНИМУМ ИЗ 3-Х СИМВОЛОВ!')
			...
			if all(total) and filename != None:
				try:
					new_article = Article(name_article=name_article, text_article=text_article, photo_article=filename, pub_date=eet_time)
					db.session.add(new_article)
					db.session.commit()
					flash('СТАТЬЯ УСПЕШНО ДОБАВЛЕНА!', category='success_article')
				except Exception as ex:
					print(ex)
			else:
				flash('НУЖНО ЗАГРУЗИТЬ ФОТО!', category='error_article')
			return redirect(url_for('admin_article'))
	return render_template('admin_article.html', check_total=Checking.check_admin(), form=form)
...
def store_news():
	...
	news = Article.query.order_by(desc(Article.pub_date)).paginate(page=page, per_page=3, error_out=False)
	pagination = Pagination(page=page, total=news.total, per_page=3, bs_version=4)
	...
	return render_template('store_news.html', form=delete_form, news=news.items, pagination=pagination, total_pages=news.pages, check_total=Checking.check_admin())
...
def flowers():
	...
	page = request.args.get('page', type=int, default=1)
	products = Item.query.filter_by(category=1).paginate(page=page, per_page=12, error_out=False)
	pagination = Pagination(page=page, total=products.total, per_page=12, bs_version=4)
	...
	return render_template('flowers.html', form=delete_form, products=products.items, pagination=pagination, total_pages=products.pages, check_total=Checking.check_admin())
...
def bouquets():
	app.logger.info('Входящий запрос: %s %s', request.method, request.path)
	page = request.args.get('page', type=int, default=1)
	products = Item.query.filter_by(category=2).paginate(page=page, per_page=9, error_out=False)
	pagination = Pagination(page=page, total=products.total, per_page=9, bs_version=4)
	...
	return render_template('bouquets.html', form=delete_form, products=products.items, pagination=pagination, total_pages=products.pages, check_total=Checking.check_admin())
...
def baskets():
	app.logger.info('Входящий запрос: %s %s', request.method, request.path)
	page = request.args.get('page', type=int, default=1)
	products = Item.query.filter_by(category=3).paginate(page=page, per_page=9, error_out=False)
	pagination = Pagination(page=page, total=products.total, per_page=9, bs_version=4)
	...
	return render_template('baskets.html', form=delete_form, products=products.items, pagination=pagination, total_pages=products.pages, check_total=Checking.check_admin())
...
def indoor():
	app.logger.info('Входящий запрос: %s %s', request.method, request.path)
	page = request.args.get('page', type=int, default=1)
	products = Item.query.filter_by(category=4).paginate(page=page, per_page=9, error_out=False)
	pagination = Pagination(page=page, total=products.total, per_page=9, bs_version=4)
	...
	return render_template('indoor.html', form=delete_form, products=products.items, pagination=pagination, total_pages=products.pages, check_total=Checking.check_admin())
...
def artificial():
	app.logger.info('Входящий запрос: %s %s', request.method, request.path)
	page = request.args.get('page', type=int, default=1)
	products = Item.query.filter_by(category=5).paginate(page=page, per_page=9, error_out=False)
	pagination = Pagination(page=page, total=products.total, per_page=9, bs_version=4)
	...
	return render_template('artificial.html', form=delete_form, products=products.items, pagination=pagination, total_pages=products.pages, check_total=Checking.check_admin())
...
def wreaths():
	app.logger.info('Входящий запрос: %s %s', request.method, request.path)
	page = request.args.get('page', type=int, default=1)
	products = Item.query.filter_by(category=6).paginate(page=page, per_page=9, error_out=False)
	pagination = Pagination(page=page, total=products.total, per_page=9, bs_version=4)
	...
	return render_template('wreaths.html', form=delete_form, products=products.items, pagination=pagination, total_pages=products.pages, check_total=Checking.check_admin())
...
def toys():
	app.logger.info('Входящий запрос: %s %s', request.method, request.path)
	page = request.args.get('page', type=int, default=1)
	products = Item.query.filter_by(category=7).paginate(page=page, per_page=9, error_out=False)
	pagination = Pagination(page=page, total=products.total, per_page=9, bs_version=4)
	...
	return render_template('toys.html', form=delete_form, products=products.items, pagination=pagination, total_pages=products.pages, check_total=Checking.check_admin())
...
def fireworks():
	app.logger.info('Входящий запрос: %s %s', request.method, request.path)
	page = request.args.get('page', type=int, default=1)
	products = Item.query.filter_by(category=8).paginate(page=page, per_page=9, error_out=False)
	pagination = Pagination(page=page, total=products.total, per_page=9, bs_version=4)
	...
	return render_template('fireworks.html', form=delete_form, products=products.items, pagination=pagination, total_pages=products.pages, check_total=Checking.check_admin())
